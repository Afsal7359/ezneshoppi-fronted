...
export default function AdminCoupons() {
  ...
  const load = () => API.coupons().then(({ data }) => setItems(data.items || []));
  useEffect(() => { load(); }, []);
    ...
    <div>
      ...
      {editing && (
        <div className="card p-5 mb-6 grid sm:grid-cols-2 gap-3">
          ...
          <div><label className="label">Type</label><select className="field" value={editing.type} onChange={(e) => setEditing({ ...editing, type: e.target.value })}><option value="percent">Percent</option><option value="fixed">Fixed</option></select></div>
      ...
      <div className="card overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-ink-900/[0.03] text-left text-xs uppercase text-ink-500">
            <tr><th className="p-3">Code</th><th className="p-3">Value</th><th className="p-3">Used</th><th className="p-3">Expires</th><th className="p-3">Status</th><th className="p-3 text-right">Actions</th></tr>
          </thead>
          <tbody>
            {items.map((c) => (
              <tr key={c._id} className="border-t border-ink-900/5">
                ...
                <td className="p-3"><span className={`chip ${c.isActive ? 'bg-green-50 text-green-700' : 'bg-ink-100'}`}>{c.isActive ? 'Active' : 'Inactive'}</span></td>
